...
def translate_with_score(
    text: str, source_lang: str = "中文", target_lang: str = "English", style: str = "free"
) -> dict:
    """翻译并评分。"""
    # 第一步：翻译（复用 step3 的 translate 函数）

    translated = step3_style.translate(text, source_lang, target_lang, style)

    # 第二步：评分
    scores = score_translation(text, translated, source_lang, target_lang, style)

    return {"translation": translated, "scores": scores}


# 测试
if __name__ == "__main__":
    result = translate_with_score("这个项目的执行需要各方协同", "中文", "English", "business")
